...
const DashBoard = () => {
  ...
  const deleteTask = async (id) => {
    ...
    try {
      const res = await axios.delete(`${process.env.REACT_APP_API_URL}/api/v1/users/${id}`);
      // console.log("res",res);
  ...
  const onSaveHandler = async (obj) => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/users/create-task`, obj);
  ...
  useEffect(() => {
    ...
    const onSortHandler = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/users/current-user-week/${by}`);
        const data = response.data.data
  ...
  useEffect(() => {
    const getCurrentUserTasks = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/users/current-user-task-todo`);
