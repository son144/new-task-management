...
const Card = ({ data, deleteTask, status, updateTodoData, editTodoData }) => {
  ...
  const toggleSubTodo = async (index, id) => {
    ...
    try {
      const res = await axios.put(`${process.env.REACT_APP_API_URL}/api/v1/users//update-subTodo/${id}`, { subTodos: updatedSubTodos })
      // console.log(res,"res");
    } catch (error) {
  ...
  const changeTaskStatus = async (id, status) => {
    ...
    try {
      const res = await axios.put(`${process.env.REACT_APP_API_URL}/api/v1/users/${id}`, { status: status });
  ...
  const editTaskHandler = async (id, obj) => {
    ...
    try {
      const res = await axios.put(`${process.env.REACT_APP_API_URL}/api/v1/users/current-user/${id}`, obj);
  ...
  const onShareHandler = async (id) => {
    ...
    const url = process.env.REACT_APP_TASK_SHARED_URL + "/shared-task/" + id;
    console.log(url, "url");
      ...
      <div className={styles.options}>
        <div className={styles.pricontainer}>
          <div className={styles.dot}
            style={{
              backgroundColor: ((task?.priority === "LOW PRIORITY") && "#63C05B") || 
              ((task?.priority === "MODERATE PRIORITY") && "#17A2B8") ||
